...
function isCollapseMenu() {
    ...
    Array.from(collapses).forEach(function (collapse) {
        ...
        // Remove event listeners added by Bootstrap
        collapse.removeEventListener("show.bs.collapse", collapseInstance.show);
        collapse.removeEventListener("hide.bs.collapse", collapseInstance.hide);

        // Show collapse
        var showCollapse = function () {
            collapseInstance.show();
        };
        collapse.addEventListener("click", showCollapse);

        // Hide collapse
        var hideCollapse = function () {
            collapseInstance.hide();
        };
        var collapseToggler = collapse.parentElement.querySelector("[data-bs-toggle='collapse']");
        if (collapseToggler) {
            collapseToggler.addEventListener("click", hideCollapse);
        }
    });
}
...
//		ul.innerHTML = '<a href="#" class="logo"><img src="assets/images/logo_1_nobg.png" alt="" height="22"></a>';
...
function initActiveMenu() {
    ...
    var currentPath = this.location.pathname;//.split("/", 2).join("/");
    ...
    if (currentPath) {
        ...
        if (a === null || a === undefined)
            a = document.getElementById("navbar-nav").querySelector('[href="' + this.location.pathname + this.location.search + '"]');
        if (a === null || a === undefined) {
            // Last try try to find the path with just the first 2 parts of the path ignore the  edit and id part
            currentPath = this.location.pathname.split("/", 2).join("/");
        ...
        if (a) {
            ...
            if (parentCollapseDiv) {
                parentCollapseDiv.classList.add("show");
                parentCollapseDiv.parentElement.classList.add("parent_active");
                parentCollapseDiv.parentElement.children[0].classList.add("active");
                parentCollapseDiv.parentElement.children[0].setAttribute("aria-expanded", "true");
					if (parentCollapseDiv.parentElement.closest('.collapse.menu-dropdown')) {
                    parentCollapseDiv.parentElement.closest(".collapse").classList.add("show");
                    ...
                    if (!emptyNotificationElem) {
                        elem.innerHTML += '<div class="empty-notification-elem">\
							...
							<div class="text-center pb-5 mt-2">\
								<h6 class="fs-18 fw-semibold lh-base">No more notifications </h6>\
							</div>\
						</div>'
                    }
                }
...
function switchTheme() {
    var themeName = window.localStorage.getItem("dx-theme");
    if (themeName === null) {
        themeName = "generic.light.compact";
        window.localStorage.setItem("dx-theme", themeName);
    }

    themeName = window.localStorage.getItem("dx-theme");
    if (themeName === "generic.light.compact")
...
var theme = window.localStorage.getItem("dx-theme");
if (theme === null) {
